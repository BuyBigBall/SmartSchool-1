...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function countUnpaidFees($student_session_id) {
        $sql  = " SELECT COUNT(*) AS unpaid_fee_cnt ";  
        $sql .= " FROM `student_fees_master` "; 
        $sql .= " INNER JOIN fee_session_groups ON fee_session_groups.id = student_fees_master.fee_session_group_id "; 
        $sql .= " INNER JOIN fee_groups_feetype ON fee_groups_feetype.fee_session_group_id = fee_session_groups.id "; 
        $sql .= " LEFT JOIN student_fees_deposite ON student_fees_deposite.student_fees_master_id=student_fees_master.id AND student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id ";
        $sql .= " WHERE `student_session_id` = " . $this->db->escape($student_session_id) . " AND student_fees_deposite.`amount_detail` IS NULL ";
        $sql .= " ORDER BY `student_fees_master`.`id` ";
        $query = $this->db->query($sql);
        return $query->result_array();
    }

    public function getUnpaidFees($student_session_id) {
        $sql  = " SELECT `student_fees_master`.id "; 
        $sql .= " ,fee_groups.name ";
        $sql .= " ,feetype.type,feetype.code ";
        $sql .= " ,fee_groups_feetype.due_date ";
        $sql .= " , `student_fees_master`.amount ";
        $sql .= " FROM `student_fees_master` "; 
        $sql .= " INNER JOIN fee_session_groups ON fee_session_groups.id = student_fees_master.fee_session_group_id "; 
        $sql .= " INNER JOIN fee_groups ON fee_groups.id=fee_session_groups.fee_groups_id ";
        $sql .= " INNER JOIN fee_groups_feetype ON fee_groups_feetype.fee_session_group_id = fee_session_groups.id ";
        $sql .= " INNER JOIN feetype ON feetype.id=fee_groups_feetype.feetype_id "; 
        $sql .= " LEFT JOIN student_fees_deposite ON student_fees_deposite.student_fees_master_id=student_fees_master.id AND student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id ";
        $sql .= " WHERE `student_session_id` = 2 AND student_fees_deposite.`amount_detail` IS NULL ";
        $sql .= " ORDER BY `student_fees_master`.`id` ";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function get_upload_proof($array) {
        return $this->db->select('*')->from('student_fees_deposite')->where($array)->get()->result_array();
    }
