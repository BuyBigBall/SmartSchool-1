...
class Notification_model extends MY_Model {
    ...
    public function getSection($arr) {
        ...
        $query = $this->db->where_in("id", $arr)->get("sections");
        return $query->result_array();
    }

    public function getNotificationSections($notification_id)
    {
        $query = $this->db->where_in("send_notification_id", $notification_id)->get("notification_roles");
        return $query->result_array();
    }
    public function getClass($notice_id) {

        $query = $this->db->where_in("send_notification_id", $notice_id)->get("notification_roles");
        $section_ids = $query->result_array();
        if( count($section_ids)>0)
        {
            $section_id = $section_ids[0]['role_id'];
            $query = $this->db->where_in("section_id", $section_id)->get("class_sections");
            $class_id = $query->result_array();
            return $class_id[0]['class_id'];
        }
        return 0;
    }
    ...
    public function getNotificationForStudent($studentid = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("SELECT send_notification.attached_file, send_notification.filename, 
...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.student_id=" . $this->db->escape($studentid) . " where send_notification.visible_student='Yes' order by send_notification.publish_date desc");
    ...
    public function getNotificationForSection($studentid = null, $section_id = null) {
        $date = date('Y-m-d');
        $query = $this->db->query("SELECT send_notification.attached_file, send_notification.filename, 
send_notification.id,send_notification.title,send_notification.publish_date,send_notification.date,send_notification.message,
IF ((read_notification.id IS NULL ),'unread','read') as notification_id
FROM send_notification
INNER JOIN notification_roles ON notification_roles.send_notification_id=send_notification.id
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.student_id=" . $this->db->escape($studentid) . " where notification_roles.role_id=" . $this->db->escape($section_id) . " order by send_notification.publish_date desc");
        return $query->result_array();
    }
