...
class Session_model extends MY_Model {
    ...
    public function getSessionIdByName($session_name) {
        $sql = "select id from sessions where session='".$session_name."'";
        $query = $this->db->query($sql);
        $rows = $query->result_array();
        if(count($rows)==0) return 0;
        return $rows[0]['id'];
    }
    public function getSessionNameById($session_id) {
        $sql = "select session from sessions where id='".$session_id."'";
        $query = $this->db->query($sql);
        $rows = $query->result_array();
