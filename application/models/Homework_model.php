...
class Homework_model extends MY_model {
    ...
    public function get($id = null) {
        ...
        if (!empty($id)) {
            $this->db->select("examtypes.name as typename, examgrades.name as gradename, `homework`.*,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name as subject_name,subject_groups.id as subject_groups_id,subject_groups.name,(select count(*) as total from submit_assignment where submit_assignment.homework_id=homework.id) as assignments");
            ...
            $this->db->join("subject_groups", "subject_group_subjects.subject_group_id=subject_groups.id", 'left');
            $this->db->join("examtypes", "examtypes.id=homework.type_id", 'left');
            $this->db->join("examgrades", "examgrades.id=homework.grade_id", 'left');
            ...
            $this->db->select("examtypes.name as typename, examgrades.name as gradename, `homework`.*,classes.class,sections.section,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name as subject_name,subject_groups.id as subject_groups_id,subject_groups.name,(select count(*) as total from submit_assignment where submit_assignment.homework_id=homework.id) as assignments");
            ...
            $this->db->join("examtypes", "examtypes.id=homework.type_id");
            $this->db->join("examgrades", "examgrades.id=homework.grade_id");
    ...
    public function getRecord($id = null) {
        ...
        $query = $this->db->
            select("submit_assignment.docs as submit_attached, homework.*,classes.class,sections.section,subjects.name,subject_groups.name as subject_group,IFNULL(submit_assignment.`created_at`,'') AS submit_assignment_date,IFNULL(examtypes.`name`,'') AS exam_type_name")->
                ...
                join('submit_assignment', 'submit_assignment.homework_id=homework.id','left')->
                join('examtypes', 'examtypes.id=homework.type_id','left')->
            where("homework.id", $id)->
            get("homework");

        return $query->row_array();
    }
    public function getStudents($id, $section_id) {
    ...
    public function addEvaluation($insert_prev, $insert_array, $homework_id, $evaluation_date, $evaluated_by, $eval_messages, $eval_point) {
        $this->db->trans_start();       # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        ...
        //for eval_messages
        foreach($eval_messages as $student_id=>$note)
        {
            $evaluation = array('date' => $evaluation_date, 'note'=>$note, 'point'=>$eval_point, 'point'=>$eval_point, 'evaluated_by'=>$evaluated_by);
            $this->db->
                where( array("homework_id"=> $homework_id, "student_session_id"=> $student_id))->
                update("homework_evaluation", $evaluation);
            // echo $this->db->last_query();              // don't echo because raising json fail
        }
        //
        // $homework = array('date' => $evaluation_date);
        // $this->db->where("homework_id", $homework_id)->update("homework_evaluation", $homework);
        // echo $this->db->last_query(); 
        //======================Code End==============================

        $this->db->trans_complete();                # Completing transaction
        if ($this->db->trans_status() === false) {  # Optional 
    ...
    public function getStudentHomeworkWithStatus($class_id, $section_id, $student_session_id) {
        $sql = "SELECT `homework`.*,IFNULL(submit_assignment.`created_at`,'') AS submit_assignment_date,IFNULL(homework_evaluation.id,0) as homework_evaluation_id,homework_evaluation.`date` as evaluationdate, IFNULL(homework_evaluation.`point`,0) AS student_point, `classes`.`class`, `sections`.`section`, `subject_group_subjects`.`subject_id`, `subject_group_subjects`.`id` as `subject_group_subject_id`, `subjects`.`name` as `subject_name`, `subject_groups`.`id` as `subject_groups_id`, `subject_groups`.`name`, IFNULL(examgrades.`gradepoint`,0) AS grade_point FROM `homework` LEFT JOIN submit_assignment ON homework.`id` = submit_assignment.`homework_id` LEFT JOIN homework_evaluation on homework_evaluation.homework_id=homework.id and homework_evaluation.student_session_id=" . $this->db->escape($student_session_id) . " LEFT JOIN examgrades ON homework.`grade_id` = examgrades.`id`  JOIN `classes` ON `classes`.`id` = `homework`.`class_id` JOIN `sections` ON `sections`.`id` = `homework`.`section_id` JOIN `subject_group_subjects` ON `subject_group_subjects`.`id` = `homework`.`subject_group_subject_id` JOIN `subjects` ON `subjects`.`id` = `subject_group_subjects`.`subject_id` JOIN `subject_groups` ON `subject_group_subjects`.`subject_group_id`=`subject_groups`.`id` WHERE `homework`.`class_id` = " . $this->db->escape($class_id) . " AND `homework`.`section_id` = " . $this->db->escape($section_id) . " AND `homework`.`session_id` = " . $this->current_session . " order by homework.homework_date desc";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function get_upload_docs($array) {
        return $this->db->
            select('homework.description as teachernote,submit_assignment.*')->
            from('submit_assignment')->
            join('homework', 'homework.id=submit_assignment.homework_id', 'left')->
            where($array)->get()->result_array();
    }
    ...
    public function search_homework_evaluation($class_id, $section_id, $subject_group_id, $subject_id) {
        if ((!empty($class_id)) && (!empty($section_id)) && (!empty($subject_id)) && (!empty($subject_group_id))) {
        ...
        $this->db->select("submit_assignment.student_id as student_id, submit_assignment.file_name as submit_attached, staff.name as markedname ,submit_assignment.id as submitid, students.firstname as stdudentname, submit_assignment.created_at as submiteddate, homework_evaluation.date as evaluationdate, homework_evaluation.point as studentpoint, examgrades.name, examgrades.gradepoint, students.firstname, `homework`.*,classes.class,sections.section,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name as subject_name,subject_groups.id as subject_groups_id,subject_groups.name");
        $this->db->join("classes", "classes.id = homework.class_id");
        $this->db->join("sections", "sections.id = homework.section_id");
        $this->db->join("subject_group_subjects", "subject_group_subjects.id = homework.subject_group_subject_id");
        $this->db->join("subjects", "subjects.id = subject_group_subjects.subject_id");
        $this->db->join("subject_groups", "subject_group_subjects.subject_group_id=subject_groups.id");
        $this->db->join("submit_assignment", "submit_assignment.homework_id=homework.id");
        $this->db->join("students", "students.id=submit_assignment.student_id");
        $this->db->join("examgrades", "examgrades.id=homework.grade_id", "left");
        $this->db->join("homework_evaluation", "homework_evaluation.homework_id=homework.id AND homework_evaluation.student_session_id=submit_assignment.student_id", 'left');
        $this->db->join("staff", "staff.id=homework_evaluation.evaluated_by","left");
                         
        //,(select count(*) as total from submit_assignment where submit_assignment.homework_id=homework.id) as assignments
    ...
    public function examreport($class_id, $section_id, $subject_group_id, $subject_id, $session_id, $groupflag) {
        if ((!empty($class_id)) && (!empty($section_id)) && (!empty($subject_id)) && (!empty($subject_group_id))) {
        ...
        $this->db->select("students.id as student_id, examtypes.name as examtypename, students.firstname as stdudentname, submit_assignment.id as submitid,  homework_evaluation.point as studentpoint, submit_assignment.file_name as submit_attached, staff.name as markedname ,submit_assignment.created_at as submiteddate, homework_evaluation.date as evaluationdate, examgrades.name, examgrades.gradepoint, students.firstname, `homework`.*,classes.class,sections.section,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name as subject_name,subject_groups.id as subject_groups_id,subject_groups.name");
        $this->db->join("classes", "classes.id = homework.class_id");
        $this->db->join("sections", "sections.id = homework.section_id");
        $this->db->join("subject_group_subjects", "subject_group_subjects.id = homework.subject_group_subject_id");
        $this->db->join("subjects", "subjects.id = subject_group_subjects.subject_id");
        ...
        $this->db->join("students", "students.id=submit_assignment.student_id");
        $this->db->join("examgrades", "examgrades.id=homework.grade_id", 'left');
        $this->db->join("homework_evaluation", "homework_evaluation.homework_id=homework.id", 'left');
        $this->db->join("examtypes", "examtypes.id=homework.type_id", 'left');
        ...
        $this->db->order_by( array('students.id'=>'asc', 'examtypes.name'=>'asc', 'homework.homework_date'=>'ASC' ) );
        $query = $this->db->get("homework");
    ...
    public function getExamStudents($homework_id)
    {
        $sql = "SELECT submit_assignment.message as submitmessage, students.id as student_id, IFNULL(homework_evaluation.id,0) as homework_evaluation_id,students.firstname,students.middlename,students.lastname,students.admission_no 
                from submit_assignment 
                INNER JOIN 
                (SELECT homework.id as homework_id,homework.class_id,homework.section_id,homework.session_id FROM `homework` 
                        WHERE id= " . $this->db->escape($homework_id) . " ) as home_work 
                        on 		home_work.homework_id=submit_assignment.homework_id 
                INNER JOIN  students 
                    on students.id=submit_assignment.student_id and students.is_active='yes' 
                LEFT JOIN homework_evaluation 
                    on 		homework_evaluation.homework_id=home_work.homework_id
                WHERE home_work.homework_id=" . $this->db->escape($homework_id) . "   
                order by students.id desc";
        //print($sql);
        $query = $this->db->query($sql);
        return $query->result_array();
    }
        public function getIncompletedExamForNotification($class_id, $section_id, $student_session_id){
        $sql  = " SELECT `subjects`.`name` AS `subject_name`,COUNT(*) AS cnt ";
        $sql .= " FROM `homework` "; 
        $sql .= " LEFT JOIN homework_evaluation ON homework_evaluation.homework_id=homework.id AND homework_evaluation.student_session_id=" . $this->db->escape($student_session_id) . " "; 
        $sql .= " JOIN `subject_group_subjects` ON `subject_group_subjects`.`id` = `homework`.`subject_group_subject_id` "; 
        $sql .= " JOIN `subjects` ON `subjects`.`id` = `subject_group_subjects`.`subject_id` "; 
        $sql .= " WHERE `homework`.`class_id` = " . $this->db->escape($class_id) . " AND `homework`.`section_id` = " . $this->db->escape($section_id) . " "; 
        $sql .= " AND `homework`.`session_id` = " . $this->current_session . " AND homework_evaluation.id IS NULL "; 
        $sql .= " GROUP BY subjects.id ";
        $sql .= " ORDER BY homework.homework_date DESC ";
        $query = $this->db->query($sql);
        return $query->result_array();
    }

    public function countIncompletedExam($class_id, $section_id, $student_session_id){
        $sql  = " SELECT COUNT(*) AS cnt ";
        $sql .= " FROM `homework` "; 
        $sql .= " LEFT JOIN homework_evaluation ON homework_evaluation.homework_id=homework.id AND homework_evaluation.student_session_id=" . $this->db->escape($student_session_id) . " "; 
        $sql .= " WHERE `homework`.`class_id` = " . $this->db->escape($class_id) . " AND `homework`.`section_id` = " . $this->db->escape($section_id) . " AND `homework`.`session_id` = " . $this->current_session . " ";
        $sql .= " AND homework_evaluation.id IS NULL ";
        $query = $this->db->query($sql);        
        return $query->result_array();
    }

    public function getEvaluationRecord($homework_id, $student_id) {
        //, he.*
        $sql  = " SELECT students.firstname, students.middlename, students.lastname, students.admission_no";
        $sql .= ", he.id as homework_evaluation_id, he.note as eval_message , he.date as eval_date, he.evaluated_by, he.point as eval_point";
        $sql .= ", sa.message as submitmessage, sa.student_id as student_id";
        $sql .= " FROM homework";
        $sql .= " LEFT JOIN submit_assignment as sa ON sa.homework_id=homework.id";
        $sql .= " LEFT JOIN homework_evaluation as he ON he.homework_id=homework.id AND sa.student_id=he.student_session_id";
        $sql .= " LEFT JOIN students ON students.id=sa.student_id";
        $sql .= " WHERE homework.id = " . $this->db->escape($homework_id);
        $sql .= " AND sa.student_id = " . $this->db->escape($student_id);
        //print($sql); exit;
        $query = $this->db->query($sql);
        return $query->row_array();
    }

    public function getExamResult($class_id, $section_id){

        $sql  = " SELECT homework.`id`, homework.`exam_name` ";
        $sql .= " ,examtypes.`id` AS exam_type_id ";
        $sql .= " ,examtypes.`name` AS exam_type_name ";
        $sql .= " ,subjects.`name` AS subject_name ";
        $sql .= " ,examgrades.`gradepoint`,examgrades.`name` AS grade_name ";
        $sql .= " ,IFNULL(homework_evaluation.`point`,0) AS student_point ";
        $sql .= " ,IFNULL(homework_evaluation.`note`,'') AS teacher_note ";
        $sql .= " ,IFNULL(homework_evaluation.`status`,'Incomplete') AS evaluation_status ";
        $sql .= " ,IFNULL(submit_assignment.`message`,'') AS student_note ";
        $sql .= " FROM homework ";
        $sql .= " LEFT JOIN examtypes ON homework.`type_id` = examtypes.`id` ";
        $sql .= " LEFT JOIN subject_group_subjects ON homework.`subject_group_subject_id` = subject_group_subjects.`id` ";
        $sql .= " LEFT JOIN subjects ON subject_group_subjects.`subject_id` = subjects.`id` ";
        $sql .= " LEFT JOIN examgrades ON homework.`grade_id` = examgrades.`id` ";
        $sql .= " LEFT JOIN homework_evaluation ON homework.`id` = homework_evaluation.`homework_id` ";
        $sql .= " LEFT JOIN submit_assignment ON homework.`id` = submit_assignment.`homework_id` ";
        $sql .= " WHERE homework.`class_id`=" . $this->db->escape($class_id);
        $sql .= " AND homework.`section_id`=" . $this->db->escape($section_id);
        $sql .= " ORDER BY examtypes.`name` ";
        ...
        foreach($result as $value){
            $data[$value['exam_type_name']][] = $value;
        }
        return $data;
