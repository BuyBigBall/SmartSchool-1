...
class Customlib
{
    ...
    public function getIncompletedExam()
    {
        $student_current_class = $this->CI->customlib->getStudentCurrentClsSection();
        $class_id             = $student_current_class->class_id;
        $section_id           = $student_current_class->section_id;
        $student_session_id   = $student_current_class->student_session_id;
        $incompletedAry = $this->CI->homework_model->countIncompletedExam($class_id, $section_id, $student_session_id);

        $count = $incompletedAry[0]['cnt'];
        if ($count > 0) {
            return $count;
        } else {
            return false;
    ...
    public function countTotalUnreadChat(){
        $user_role = "";
        if ($this->CI->session->has_userdata('admin')) {
            $user_role = "staff";
            $userdata = $this->CI->customlib->getUserData();
            $login_user_id  = $userdata['id'];
        } else if ($this->CI->session->has_userdata('student')) {
            $user_role = "student";
            $login_user_id = $this->CI->customlib->getStudentSessionUserID();
        }
        $cnt = $this->CI->chatuser_model->countTotalUnreadMessage($login_user_id);
