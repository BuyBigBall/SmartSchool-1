...
class Teacher extends Admin_Controller {
    ...
    public function assign_class_teacher() {
        ...
        foreach($assignteacherlist as &$teacherclass)
        {
            $teacherclass['input_date'] = date($this->customlib->getSchoolDateFormat(), $this->customlib->dateYYYYMMDDtoStrtotime($teacherclass['created_at']));
            $teacherclass['session'] = $this->session_model->getSessionNameById($teacherclass['session_id']);
        }
