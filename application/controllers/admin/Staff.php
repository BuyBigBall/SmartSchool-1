            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if ($ext == 'xlsx') {
                    ...
                    foreach ($cell_collection as $cell) {
                        ...
                        //The header will/should be in 2 rows. 
                        if ($row <= 2) {
                            $header[$row][$column] = $data_value;
                        } else {
                            $result[$row][$column] = $data_value;
                            if($column==2)  //for full name
                            {
                                $result[$row][1] = $result[$row][1] . ' ' . $result[$row][2];
                                $result[$row][2] = '';
                            }
                        }
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $r_key => $r_value) {
                            ...
                            $check_exists      = $this->staff_model->import_check_data_exists($r_value['name'], $r_value['employee_id']);
                            $check_emailexists = $this->staff_model->import_check_email_exists($r_value['name'], $r_value['employee_id']);

                            if ($check_exists == 0 && $check_emailexists == 0) {

                                $staff_info['employee_id']          = $this->encoding_lib->toUTF8($r_value[0]);
                                $staff_info['name']                 = $this->encoding_lib->toUTF8($r_value[1]);
                                $staff_info['surname']              = $this->encoding_lib->toUTF8($r_value[2]);
                                $staff_info['father_name']          = $this->encoding_lib->toUTF8($r_value[3]);
                                $staff_info['mother_name']          = $this->encoding_lib->toUTF8($r_value[4]);
                                $staff_info['email']                = $this->encoding_lib->toUTF8($r_value[5]);
                                $staff_info['gender']               = $this->encoding_lib->toUTF8($r_value[6]);
                                $staff_info['dob']                  = $this->encoding_lib->toUTF8($r_value[7]);
                                $staff_info['date_of_joining']      = $this->encoding_lib->toUTF8($r_value[8]);
                                $staff_info['contact_no']           = $this->encoding_lib->toUTF8($r_value[9]);
                                $staff_info['emergency_contact_no'] = $this->encoding_lib->toUTF8($r_value[10]);
                                $staff_info['marital_status']       = $this->encoding_lib->toUTF8($r_value[11]);
                                $staff_info['local_address']        = $this->encoding_lib->toUTF8($r_value[12]);
                                $staff_info['permanent_address']    = $this->encoding_lib->toUTF8($r_value[13]);
                                // $staff_info['qualification']        = $this->encoding_lib->toUTF8($r_value[14]);
                                // $staff_info['work_exp']             = $this->encoding_lib->toUTF8($r_value[15]);
                                $staff_info['note']                 = $this->encoding_lib->toUTF8($r_value[16]);
                                //$staff_info['date_of_leaving']      = $this->encoding_lib->toUTF8($r_value['date_of_leaving']);
                                //$staff_info['account_title']        = $this->encoding_lib->toUTF8($r_value['account_title']);
                                //$staff_info['bank_account_no']      = $this->encoding_lib->toUTF8($r_value['bank_account_no']);
                                //$staff_info['bank_name']            = $this->encoding_lib->toUTF8($r_value['bank_name']);
                                //$staff_info['ifsc_code']            = $this->encoding_lib->toUTF8($r_value['ifsc_code']);
                                //$staff_info['payscale']             = $this->encoding_lib->toUTF8($r_value['payscale']);
                                //$staff_info['basic_salary']         = $this->encoding_lib->toUTF8($r_value['basic_salary']);
                                //$staff_info['epf_no']               = $this->encoding_lib->toUTF8($r_value['epf_no']);
                                //$staff_info['contract_type']        = $this->encoding_lib->toUTF8($r_value['contract_type']);
                                //$staff_info['shift']                = $this->encoding_lib->toUTF8($r_value['shift']);
                                //$staff_info['location']             = $this->encoding_lib->toUTF8($r_value['location']);
                                //$staff_info['facebook']             = $this->encoding_lib->toUTF8($r_value['facebook']);
                                //$staff_info['twitter']              = $this->encoding_lib->toUTF8($r_value['twitter']);
                                //$staff_info['linkedin']             = $this->encoding_lib->toUTF8($r_value['linkedin']);
                                //$staff_info['instagram']            = $this->encoding_lib->toUTF8($r_value['instagram']);
                                //$staff_info['resume']               = $this->encoding_lib->toUTF8($r_value['resume']);
                                //$staff_info['joining_letter']       = $this->encoding_lib->toUTF8($r_value['joining_letter']);
                                //$staff_info['resignation_letter']   = $this->encoding_lib->toUTF8($r_value['resignation_letter']);
                                $staff_info['user_id']              = $this->input->post('role');
                                $staff_info['designation']          = $this->input->post('designation');
                                $staff_info['department']           = $this->input->post('department');
                                $staff_info['is_active']            = 1;

                                $password = $this->role->get_random_password($chars_min = 6, $chars_max = 6, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);

                                $staff_info['password'] = $this->enc_lib->passHashEnc($password);

                                $role_array = array('role_id' => $this->input->post('role'), 'staff_id' => 0);

                                $insert_id = $this->staff_model->batchInsert($staff_info, $role_array);
                                $staff_id  = $insert_id;
                                if ($staff_id) {

                                    $teacher_login_detail = array('id' => $staff_id, 'credential_for' => 'staff', 'username' => $staff_info['email'], 'password' => $password, 'contact_no' => $staff_info['contact_no'], 'email' => $staff_info['email']);
