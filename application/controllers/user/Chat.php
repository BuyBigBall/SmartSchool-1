...
class Chat extends Student_Controller {
    ...
    public function myuser() {
        ...
        $role = $this->session->userdata('student')['role'];
        if($role=='parent')
            $chat_user = $this->chatuser_model->getMyID($student_id, 'parent');
        else
        ...
        if (!empty($chat_user)) {
            ...
            $data['userList'] = $this->chatuser_model->myUser($student_id, $chat_user->id, $chat_user->user_type);
        }
    ...
    function mynewuser() {
        ...
        if (!empty($chat_user)) {
            $data['new_user_list'] = $this->chatuser_model->mynewuser($chat_user->id, $users_list, $chat_user->user_type);
        }
