  ...
  $data['exam_result']          = $this->examgroupstudent_model->searchStudentExams($student['student_session_id'], true, true);
    ...
    public function import()
    {
        ...
        $fields = array(0 => 'admission_no', 
                        ...
                        18 => 'father_name', 
                        19 => 'father_phone', 
                        20 => 'father_occupation', 
                        21 => 'mother_name', 
                        22 => 'mother_phone', 
                        23 => 'mother_occupation', 
                        24 => 'guardian_is', 
                        25 => 'guardian_name', 
                        26 => 'guardian_relation', 
                        27 => 'guardian_email', 
                        28 => 'guardian_phone', 
                        29 => 'guardian_occupation', 
                        30 => 'guardian_address', 
                        31 => 'current_address', 
                        32 => 'permanent_address', 
                        33 => 'bank_account_no', 
                        34 => 'bank_name', 
                        35 => 'ifsc_code', 
                        36 => 'adhar_no', 
                ...
                if ($ext == 'xlsx') {
                    ...
                    foreach ($cell_collection as $cell) {
                        ...
                        if ($row <= 2) {
                            ...
                            if($column==4)  //for full name
                            {
                                $result[$row][2] = $result[$row][2] . ' ' . $result[$row][3] . ' ' . $result[$row][4];
                                $result[$row][3] = '';
                                $result[$row][4] = '';
                            }
                        }
                    ...
                    if (!empty($result)) {
                        $rowcount = 0; $i=1;
                        foreach ($result as $rownum=>$onerow)   //for ($i = 1; $i <= count($result); $i++)
                        {   
                            $i++;
                            if( empty( $onerow[2] ) ) continue;
                            $student_data[$i] = array();
                            $n                = 0;
                            foreach($fields as $field) {
                                $student_data[$i][$field] = $this->encoding_lib->toUTF8($onerow[$n]);
                                $student_data[$i]['is_active'] = 'yes';

                                if (date('Y-m-d', strtotime($onerow['date_of_birth'])) === $onerow['date_of_birth']) {
                                    $student_data[$i]['dob'] = date('Y-m-d', strtotime($onerow['date_of_birth']));
                                } else {
                                    $student_data[$i]['dob'] = null;
                                }

                                if (date('Y-m-d', strtotime($onerow['measurement_date'])) === $onerow['measurement_date']) {
                                    $student_data[$i]['measurement_date'] = date('Y-m-d', strtotime($onerow['measurement_date']));
                                } else {
                                    $student_data[$i]['measurement_date'] = '';
                                }

                                if (date('Y-m-d', strtotime($onerow['admission_date'])) === $onerow['admission_date']) {
                                    $student_data[$i]['admission_date'] = date('Y-m-d', strtotime($onerow['admission_date']));
                                } else {
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $rowcount++;
                                $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">Total ' . count($result) . " records found in Excel file. Total " . $rowcount . ' records imported successfully.</div>');
                    ...
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">' . $this->lang->line('please_upload_XLSX_file_only') . '</div>');
                }
            }
