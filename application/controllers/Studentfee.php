...
class Studentfee extends Admin_Controller
{
    ...
    public function addfeegrp()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //---------------> uploaded file process 2021.07.01
            {
                if (isset($_FILES["prooffile"]) && !empty($_FILES['prooffile']['name'])) {
                    $time     = md5($_FILES["prooffile"]['name'] . microtime());
                    $fileInfo = pathinfo($_FILES["prooffile"]["name"]);
                    $img_name = $time . '.' . $fileInfo['extension'];


                    $upload = move_uploaded_file($_FILES["prooffile"]["tmp_name"], "./uploads/school_expense/" . $img_name);

                    if ($upload) {
                        $proof['proof_filename']      = $img_name;
                        $proof['file_name'] = $_FILES["file"]['name'];
                        //print_r($proof);
                    }
                }
            ...
            foreach ($total_row as $total_row_key => $total_row_value) {
                ...
                $collected_array[] = array(
                    ...
                    'amount_detail'          => $json_array,
                    'proof_filename'         => $proof['proof_filename']    //<--- added 2021.07.01
                );
            }
